...
extern UCHAR current_DOR;	// 当前数字输出寄存器( Digital Output Register )
...
static UCHAR reply_buffer[ MAX_REPLIES ] = {0};	// 存放FDC 返回的结果信息
...
repeat:
	...
	// 如果当前选择的软驱不是指定的软驱nr,并且已经选择其它了软驱,则让当前任务进入可中断等待状态
...
__inline VOID setup_rw_floppy()
{
	...
	output_byte( 2							);		// 发送参数( 字节数( N=2 )512 字节 )./* sector size = 512 */
	output_byte( (CHAR)floppy->sect			);		// 发送参数( 每磁道扇区数 ).
...
static VOID recal_interrupt()
{
	...
	if ( result() != 2 || ( ST0 & 0xE0 ) == 0x60 )	// 如果返回结果字节数不等于2
	{
		fp_reset = 1;								// 异常结束,则置复位标志
	}
	else
	{
		fp_recalibrate = 0;						// 否则复位重新校正标志
	}
...
static VOID recalibrate_floppy()
{
	...
	if ( fp_reset )									// 如果出错( 复位标志被置位 )则继续执行软盘请求.
	{
		do_fd_request();
	}
}
...
static VOID reset_interrupt()
{
	...
	output_byte( (UCHAR)cur_spec1 );	// hut etc */// 发送参数.
	output_byte( 6 );					// Head load time =6ms, DMA
...
static VOID floppy_on_interrupt()
{
	...
	// 定时延迟 2 个滴答时间,然后调用软盘读写传输函数transfer().否则直接调用软盘读写传输函数.
...
VOID do_fd_request()
{
	...
	{
		end_request( 0 );				// 如果block+2 大于磁盘扇区总数,则
		goto repeat;					// 结束本次软盘请求项
	}

	// 求对应在磁道上的扇区号,磁头号,磁道号,搜寻磁道号( 对于软驱读不同格式的盘 )
	sector		= (UCHAR)( block % floppy->sect );			// 起始扇区对每磁道扇区数取模,得磁道上扇区号.
	block 	   /= floppy->sect;								// 起始扇区对每磁道扇区数取整,得起始磁道数.
	head		= (UCHAR)( block % floppy->head );			// 起始磁道数对磁头数取模,得操作的磁头号.
	track		= (UCHAR)( block / floppy->head );			// 起始磁道数对磁头数取整,得操作的磁道号.
...
// 软盘系统初始化.
// 设置软盘块设备的请求处理函数( do_fd_request() ),并设置软盘中断门( LONG 0x26,对应硬件
...
VOID floppy_init()
{
	...
	set_trap_gate( 0x26, &floppy_interrupt );			//设置软盘中断门 LONG 0x26( 38 ).
