...
#define INTMASK  ( 1 << ( SIGINT -1 ) )	// 键盘中断( LONG )信号屏蔽位.
...
VOID copy_to_cooked( TTY_Struct * tty )
{
	...
	while ( !EMPTY( tty->read_q ) && !FULL( tty->secondary ) )
	{
		...
		else if ( c == 10 && I_NLCR( tty ) )// 如果该字符是换行符NL( 10 )并且换行转回车标志NLCR 置位,则将其转换为回车符CR( 13 )
		{
			c = 13;
		}

		if ( I_UCLC( tty ) )				// 如果大写转小写标志UCLC 置位,则将该字符转换为小写字符
		{
			c = tolower( c );
		}

		if ( L_CANON( tty ) )				// 如果本地模式标志集中规范( 熟 )模式标志CANON 置位,则进行以下处理
		{
			if ( c == KILL_CHAR( tty ) )	// 如果该字符是键盘终止控制字符KILL( ^U ),则进行删除输入行处理
			{
...
LONG tty_write( unsigned channel, CHAR * buf, LONG nr )
{
	...
	while ( nr>0 ) 
	{
		...
		while ( nr > 0 && !FULL( tty->write_q ) )
		{
			...
			// 用户数据缓冲指针b 前进1 字节;欲写字节数减1 字节;复位cr_flag 标志,并将该字节放入tty写队列中
			b++;
			nr--;
			cr_flag = 0;
			PUTCH( c, tty->write_q );
